#ifndef V8_HEAP_COPYING_INL_H_
#define V8_HEAP_COPYING_INL_H_
...
void Copying::PromotionList::Local::PushRegularObject(HeapObject object,
                                                        int size) {
  regular_object_promotion_list_local_.Push({object, size});
}

void Copying::PromotionList::Local::PushLargeObject(HeapObject object,
                                                      Map map, int size) {
  large_object_promotion_list_local_.Push({object, map, size});
}

size_t Copying::PromotionList::Local::LocalPushSegmentSize() const {
  return regular_object_promotion_list_local_.PushSegmentSize() +
         large_object_promotion_list_local_.PushSegmentSize();
}

bool Copying::PromotionList::Local::Pop(struct PromotionListEntry* entry) {
...
bool Copying::MigrateObject(Map map, HeapObject source, HeapObject target,
                              int size,
...
CopyAndForwardResult Copying::SemiSpaceCopyObject(
...
template <typename THeapObjectSlot,
          Copying::PromotionHeapChoice promotion_heap_choice>
CopyAndForwardResult Copying::PromoteObject(Map map, THeapObjectSlot slot,
                                              HeapObject object,
                                              int object_size,
                                              ObjectFields object_fields) {
  static_assert(std::is_same<THeapObjectSlot, FullHeapObjectSlot>::value ||
...
SlotCallbackResult Copying::RememberedSetEntryNeeded(
...
bool Copying::HandleLargeObject(Map map, HeapObject object, int object_size,
                                  ObjectFields object_fields) {
...
template <typename THeapObjectSlot,
          Copying::PromotionHeapChoice promotion_heap_choice>
SlotCallbackResult Copying::EvacuateObjectDefault(
    Map map, THeapObjectSlot slot, HeapObject object, int object_size,
    ObjectFields object_fields) {
  static_assert(std::is_same<THeapObjectSlot, FullHeapObjectSlot>::value ||
                    std::is_same<THeapObjectSlot, HeapObjectSlot>::value,
  ...
  heap()->FatalProcessOutOfMemory("Copying: semi-space copy");
  UNREACHABLE();
}

template <typename THeapObjectSlot>
SlotCallbackResult Copying::EvacuateThinString(Map map, THeapObjectSlot slot,
                                                 ThinString object,
                                                 int object_size) {
  static_assert(std::is_same<THeapObjectSlot, FullHeapObjectSlot>::value ||
                    std::is_same<THeapObjectSlot, HeapObjectSlot>::value,
...
template <typename THeapObjectSlot>
SlotCallbackResult Copying::EvacuateShortcutCandidate(Map map,
...
template <typename THeapObjectSlot>
SlotCallbackResult Copying::EvacuateInPlaceInternalizableString(
...
template <typename THeapObjectSlot>
SlotCallbackResult Copying::EvacuateObject(THeapObjectSlot slot, Map map,
                                             HeapObject source) {
  static_assert(std::is_same<THeapObjectSlot, FullHeapObjectSlot>::value ||
                    std::is_same<THeapObjectSlot, HeapObjectSlot>::value,
                "Only FullHeapObjectSlot and HeapObjectSlot are expected here");
...
template <typename THeapObjectSlot>
SlotCallbackResult Copying::CopyObject(THeapObjectSlot p,
                                             HeapObject object) {
  static_assert(std::is_same<THeapObjectSlot, FullHeapObjectSlot>::value ||
                    std::is_same<THeapObjectSlot, HeapObjectSlot>::value,
                "Only FullHeapObjectSlot and HeapObjectSlot are expected here");
...
SlotCallbackResult Copying::CheckAndCopyObject(Heap* heap, TSlot slot) {
  static_assert(
  ...
  if (Heap::InFromPage(object)) {
    ...
    SlotCallbackResult result =
        CopyObject(THeapObjectSlot(slot), heap_object);
...
void CopyingVisitor::VisitPointers(HeapObject host, ObjectSlot start,
                                    ObjectSlot end) {
  return VisitPointersImpl(host, start, end);
}

void CopyingVisitor::VisitPointers(HeapObject host, MaybeObjectSlot start,
                                    MaybeObjectSlot end) {
  return VisitPointersImpl(host, start, end);
}

void CopyingVisitor::VisitCodePointer(HeapObject host, CodeObjectSlot slot) {
...
void CopyingVisitor::VisitCodeTarget(Code host, RelocInfo* rinfo) {
...
void CopyingVisitor::VisitEmbeddedPointer(Code host, RelocInfo* rinfo) {
...
template <typename TSlot>
void CopyingVisitor::VisitHeapObjectImpl(TSlot slot, HeapObject heap_object) {
  if (Heap::InYoungGeneration(heap_object)) {
    using THeapObjectSlot = typename TSlot::THeapObjectSlot;
    copying_->CopyObject(THeapObjectSlot(slot), heap_object);
  }
}

template <typename TSlot>
void CopyingVisitor::VisitPointersImpl(HeapObject host, TSlot start,
                                        TSlot end) {
...
int CopyingVisitor::VisitJSArrayBuffer(Map map, JSArrayBuffer object) {
...
int CopyingVisitor::VisitEphemeronHashTable(Map map,
  ...
  copying_->AddEphemeronHashTable(table);
