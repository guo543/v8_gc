#include "src/heap/copying-collector.h"
...
class IterateAndCopyPromotedObjectsVisitor final : public ObjectVisitor {
 public:
  IterateAndCopyPromotedObjectsVisitor(Copying* Copying,
                                           bool record_slots)
      : copying_(Copying), record_slots_(record_slots) {}
  ...
  inline void VisitEphemeron(HeapObject obj, int entry, ObjectSlot key,
    ...
    if (ObjectInYoungGeneration(*key)) {
      ...
      copying_->RememberPromotedEphemeron(
 ...
 private:
  ...
  V8_INLINE void HandleSlot(HeapObject host, THeapObjectSlot slot,
    ...
    copying_->PageMemoryFence(MaybeObject::FromObject(target));

    if (Heap::InFromPage(target)) {
      SlotCallbackResult result = copying_->CopyObject(slot, target);
  ...
  Copying* const copying_;
  const bool record_slots_;
};

namespace {

V8_INLINE bool IsUncopiedHeapObject(Heap* heap, Object object) {
...
V8_INLINE bool IsUncopiedHeapObject(Heap* heap, HeapObject heap_object) {
  return Heap::InFromPage(heap_object) &&
         !heap_object.map_word(kRelaxedLoad).IsForwardingAddress();
}

bool IsUncopiedHeapObjectSlot(Heap* heap, FullObjectSlot p) {
  return IsUncopiedHeapObject(heap, *p);
}

}  // namespace

class CopyWeakObjectRetainer : public WeakObjectRetainer {
 public:
...
CopyingCollector::JobTask::JobTask(
    CopyingCollector* outer, std::vector<std::unique_ptr<Copying>>* copyings,
    std::vector<std::pair<ParallelWorkItem, MemoryChunk*>> memory_chunks,
    Copying::CopiedList* copied_list, Copying::PromotionList* promotion_list)
    : outer_(outer),
      copyings_(copyings),
...
void CopyingCollector::JobTask::Run(JobDelegate* delegate) {
  DCHECK_LT(delegate->GetTaskId(), copyings_->size());
  Copying* copying = (*copyings_)[delegate->GetTaskId()].get();
  if (delegate->IsJoiningThread()) {
    // This is already traced in GCTracer::Scope::SCAVENGER_SCAVENGE_PARALLEL
    // in ScavengerCollector::CollectGarbage.
    ProcessItems(delegate, copying);
  } else {
...
size_t CopyingCollector::JobTask::GetMaxConcurrency(
...
void CopyingCollector::JobTask::ProcessItems(JobDelegate* delegate,
                                             Copying* copying) {
  double scavenging_time = 0.0;
  {
    TimedScope scope(&scavenging_time);
    ConcurrentCopyPages(copying);
    copying->Process(delegate);
  }
  if (FLAG_trace_parallel_scavenge) {
    PrintIsolate(outer_->heap_->isolate(),
                 ...
                 copying->bytes_copied(), copying->bytes_promoted());
  }
}

void CopyingCollector::JobTask::ConcurrentCopyPages(Copying* copying) {
  while (remaining_memory_chunks_.load(std::memory_order_relaxed) > 0) {
    ...
    for (size_t i = *index; i < memory_chunks_.size(); ++i) {
      ...
      copying->CopyPage(work_item.second);
...
CopyingCollector::CopyingCollector(Heap* heap)
    : isolate_(heap->isolate()), heap_(heap) {}

class V8_NODISCARD MyScopedFullHeapCrashKey {
 public:
  explicit MyScopedFullHeapCrashKey(Isolate* isolate) : isolate_(isolate) {
    isolate_->AddCrashKey(v8::CrashKeyId::kDumpType, "heap");
  }
  ~MyScopedFullHeapCrashKey() {
...
#ifdef DEBUG
namespace {
class MyPrintHandleVisitor : public RootVisitor {
 public:
  void VisitRootPointer(Root root, const char* description,
                        FullObjectSlot p) override {
    PrintF("  handle %p to %p, isHeapObj: %d\n", p.ToVoidPtr(),
           reinterpret_cast<void*>((*p).ptr()), (*p).IsHeapObject());
  }
...
void CopyingCollector::CollectGarbage() {
  MyScopedFullHeapCrashKey collect_full_heap_dump_if_crash(isolate_);

  DCHECK(surviving_new_large_objects_.empty());
  std::vector<std::unique_ptr<Copying>> copyings;
  Copying::EmptyChunksList empty_chunks;
  const int num_copy_tasks = NumberOfCopyTasks();
  Copying::CopiedList copied_list;
  Copying::PromotionList promotion_list;
...

  {
    ...
    for (int i = 0; i < num_copy_tasks; ++i) {
      copyings.emplace_back(
          new Copying(this, heap_, is_logging, &empty_chunks, &copied_list,
    ...
    RootCopyingVisitor root_copying_visitor(copyings[kMainThreadId].get());
    ...
    }
    {
      ...
      heap_->IterateRoots(&root_copying_visitor, options);
      isolate_->global_handles()->IterateYoungStrongAndDependentRoots(
          &root_copying_visitor);
      copyings[kMainThreadId]->Publish();
    }
    {
      ...
      V8::GetCurrentPlatform()
          ->PostJob(v8::TaskPriority::kUserBlocking,
                    std::make_unique<JobTask>(this, &copyings,
                                              std::move(memory_chunks),
                                              &copied_list, &promotion_list))
          ->Join();*/

      // Single-threaded GC
      std::unique_ptr<JobTask> jobTask =
          std::make_unique<JobTask>(this, &copyings, std::move(memory_chunks),
                                    &copied_list, &promotion_list);
      jobTask->ProcessItems(NULL, copyings[kMainThreadId].get());
      DCHECK(copied_list.IsEmpty());
      DCHECK(promotion_list.IsEmpty());
    }

    if (V8_UNLIKELY(FLAG_scavenge_separate_stack_scanning)) {
      IterateStackAndCopy(&root_copying_visitor, &copyings,
                              kMainThreadId);
      DCHECK(copied_list.IsEmpty());
      DCHECK(promotion_list.IsEmpty());
    }
...

  {
    ...
    Copying::EmptyChunksList::Local empty_chunks_local(&empty_chunks);
    MemoryChunk* chunk;
...
void CopyingCollector::IterateStackAndCopy(
    RootCopyingVisitor* root_copying_visitor,
    std::vector<std::unique_ptr<Copying>>* copyings, int main_thread_id) {
  ...
  heap_->IterateStackRoots(root_copying_visitor);
  (*copyings)[main_thread_id]->Process();
  size_t survived_bytes_after = 0;
  for (auto& copying : *copyings) {
    survived_bytes_after += copying->bytes_copied() + copying->bytes_promoted();
  }
  TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("v8.gc"),
               "V8.GCCopyingStackScanning", "survived_bytes_before",
  ...
  if (FLAG_trace_gc_verbose && !FLAG_trace_gc_ignore_scavenger) {
    isolate_->PrintWithTimestamp(
        "Copying stack scanning: survived_before=%4zuKB, "
...
void CopyingCollector::SweepArrayBufferExtensions() {
  heap_->array_buffer_sweeper()->RequestSweep(
      ArrayBufferSweeper::SweepingType::kYoung);
}

void CopyingCollector::HandleSurvivingNewLargeObjects() {
...
void CopyingCollector::MergeSurvivingNewLargeObjects(
...
int CopyingCollector::NumberOfCopyTasks() {
  /*
...
Copying::PromotionList::Local::Local(Copying::PromotionList* promotion_list)
...
Copying::Copying(CopyingCollector* collector, Heap* heap, bool is_logging,
...
void Copying::IterateAndCopyPromotedObject(HeapObject target, Map map,
                                                 int size) {
  ...
  IterateAndCopyPromotedObjectsVisitor visitor(this, record_slots);
...
void Copying::RememberPromotedEphemeron(EphemeronHashTable table, int entry) {
  auto indices =
      ephemeron_remembered_set_.insert({table, std::unordered_set<int>()});
  indices.first->second.insert(entry);
}

void Copying::AddPageToSweeperIfNecessary(MemoryChunk* page) {
...
void Copying::CopyPage(MemoryChunk* page) {
  ...
  if (page->slot_set<OLD_TO_NEW, AccessMode::ATOMIC>() != nullptr) {
    ...
    RememberedSet<OLD_TO_NEW>::IterateAndTrackEmptyBuckets(
        ...
        [this, &filter](MaybeObjectSlot slot) {
          ...
          return CheckAndCopyObject(heap_, slot);
        },
        &empty_chunks_local_);
  }
      ...
      page, [=](SlotType type, Address addr) {
        return UpdateTypedSlotHelper::UpdateTypedSlot(
            heap_, type, addr, [this](FullMaybeObjectSlot slot) {
              return CheckAndCopyObject(heap(), slot);
            });
      });
...
void Copying::Process(JobDelegate* delegate) {
  CopyingVisitor copying_visitor(this);
  ...
  do {
    ...
    while (promotion_list_local_.ShouldEagerlyProcessPromotionList() &&
      ...
      copying_visitor.Visit(object_and_size.first);
      done = false;
    ...
    while (promotion_list_local_.Pop(&entry)) {
      ...
      IterateAndCopyPromotedObject(target, entry.map, entry.size);
      done = false;
      if (delegate && ((++objects % kInterruptThreshold) == 0)) {
...
void CopyingCollector::ProcessWeakReferences(
    EphemeronTableList* ephemeron_table_list) {
  CopyWeakObjectRetainer weak_object_retainer;
...
void CopyingCollector::ClearYoungEphemerons(
    EphemeronTableList* ephemeron_table_list) {
  ephemeron_table_list->Iterate([this](EphemeronHashTable table) {
    for (InternalIndex i : table.IterateEntries()) {
      ...
      if (IsUncopiedHeapObject(heap_, key)) {
        table.RemoveEntry(i);
      } else {
...
// entry has a dead new-space key.
void CopyingCollector::ClearOldEphemerons() {
...
void Copying::Publish() {
  copied_list_local_.Publish();
  promotion_list_local_.Publish();
}

void Copying::AddEphemeronHashTable(EphemeronHashTable table) {
  ephemeron_table_list_local_.Push(table);
}

void RootCopyingVisitor::VisitRootPointer(Root root, const char* description,
                                           FullObjectSlot p) {
...
void RootCopyingVisitor::VisitRootPointers(Root root, const char* description,
...
void RootCopyingVisitor::CopyPointer(FullObjectSlot p) {
  Object object = *p;
  DCHECK(!HasWeakHeapObjectTag(object));
  DCHECK(!MapWord::IsPacked(object.ptr()));
  if (Heap::InYoungGeneration(object)) {
    copying_->CopyObject(FullHeapObjectSlot(p), HeapObject::cast(object));
  }
}

RootCopyingVisitor::RootCopyingVisitor(Copying* copying) : copying_(copying) {}

CopyingVisitor::CopyingVisitor(Copying* copying)
    : NewSpaceVisitor<CopyingVisitor>(copying->heap()->isolate()),
      copying_(copying) {}
