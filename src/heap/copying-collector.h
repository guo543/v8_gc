#ifndef V8_HEAP_COPYING_COLLECTOR_H_
#define V8_HEAP_COPYING_COLLECTOR_H_
...
constexpr int kMyEphemeronTableListSegmentSize = 128;
using EphemeronTableList =
    ::heap::base::Worklist<EphemeronHashTable,
                           kMyEphemeronTableListSegmentSize>;
...
class Copying {
 public:
  ...
  Copying(CopyingCollector* collector, Heap* heap, bool is_logging,
  ...
  void CopyPage(MemoryChunk* page);
 ...
 private:
  ...
  inline SlotCallbackResult CheckAndCopyObject(Heap* heap, TSlot slot);

  // Scavenges an object |object| referenced from slot |p|. |object| is required
  // to be in from space.
  template <typename THeapObjectSlot>
  inline SlotCallbackResult CopyObject(THeapObjectSlot p,
                                           HeapObject object);
  ...
  void IterateAndCopyPromotedObject(HeapObject target, Map map, int size);
  void RememberPromotedEphemeron(EphemeronHashTable table, int index);

  CopyingCollector* const collector_;
  Heap* const heap_;
  ...
  friend class IterateAndCopyPromotedObjectsVisitor;
  friend class RootCopyingVisitor;
  friend class CopyingVisitor;
};

// Helper class for turning the scavenger into an object visitor that is also
// filtering out non-HeapObjects and objects which do not reside in new space.
class RootCopyingVisitor final : public RootVisitor {
 public:
  explicit RootCopyingVisitor(Copying* copying);
 ...
 private:
  void CopyPointer(FullObjectSlot p);

  Copying* const copying_;
};

class CopyingVisitor final : public NewSpaceVisitor<CopyingVisitor> {
 public:
  explicit CopyingVisitor(Copying* copying);
...
class CopyingCollector {
 public:
  static const int kMaxCopyingTasks = 8;
  static const int kMainThreadId = 0;

  explicit CopyingCollector(Heap* heap);
 ...
 private:
  class JobTask : public v8::JobTask {
   ...
   private:
    void ProcessItems(JobDelegate* delegate, Copying* copying);
    void ConcurrentCopyPages(Copying* copying);

    CopyingCollector* outer_;

    std::vector<std::unique_ptr<Copying>>* copyings_;
    std::vector<std::pair<ParallelWorkItem, MemoryChunk*>> memory_chunks_;
    std::atomic<size_t> remaining_memory_chunks_{0};
    IndexGenerator generator_;
    
    Copying::CopiedList* copied_list_;
    Copying::PromotionList* promotion_list_;
...
#endif  // V8_HEAP_COPYING_COLLECTOR_H_
